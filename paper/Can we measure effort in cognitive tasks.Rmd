...
thanks: |
  Preprint (DOI: [10.31234/osf.io/6pvht](https://psyarxiv.com/6pvht/)), not peer-reviewed. Invited paper submitted to Cortex special issue "[Strengthening derivation chains in cognitive neuroscience](https://www.journals.elsevier.com/cortex/call-for-papers/special-issue-on-strengthening-derivation-chains-in-cognitive-neuroscience)". All data, materials, and code is available in the GitHub (see https://github.com/jamessteeleii/cognitive_effort) repository or Open Science Framework (see https://osf.io/yfam5/) for this project. 
...
Generally speaking, the application of an IRT model allows for test data to be decomposed into an estimate of a characteristic of the individual (i.e., their 'ability', $\theta_p$). Additionally, the other parameters of such models which reflect the item characteristics can also be estimated such as the items location on the scales (i.e., their 'difficulty', $\beta_i$). It is these two parameters that interest me and which I will draw upon. Two of the most simple and closely related models both incorporating ability and difficulty parameters are the Guttman [-@guttman_basis_1944] and Rasch [-@rasch_studies_1960] models. 
...
Where $P_{pi}(success)$ denotes the probability of a person $p$ successfully performing the item $i$ attempted. The relationship between the probability of success and a persons ability level is represented by a step function (see figure \@ref(fig:guttman-rasch-ICC-plot)A) and this model is said to describe a deterministic process.  
...
The Rasch model is essentially a stochastic version of the Guttman model where the relationship between the probability of success and a persons ability level is represented by a logistic function (see figure \@ref(fig:guttman-rasch-ICC-plot)B). It is represented formally as: 
...
guttman_prob$` ` <- recode(guttman_prob$` `, "1" = "$\\theta_1$", "2"= "$\\theta_2$", "3" = "$\\theta_3$")
...
rasch_prob$` ` <- recode(rasch_prob$` `, "1" = "$\\theta_1$", "2"= "$\\theta_2$", "3" = "$\\theta_3$")
...
\begin{equation}
\ MADP_S=\Biggl \lfloor \sum^N_{n=1}\frac{abs(S_p-\hat{S}_p)}{S_p}\Biggr \rfloor/N
(\#eq:MADPs-eq)
\end{equation}

\begin{equation}
\ MADP_L=\Biggl \lfloor \sum^L_{k=1}\frac{abs(L_p-\hat{L}_p)}{L_p}\Biggr \rfloor/K
(\#eq:MADPl-eq)
\end{equation}
...
\begin{equation}
\ E_{Api,logit-diff}=(\hat{\theta_p} - \hat{\beta_i})\times-1, \textrm{ if and only if } \hat{\theta_p} \geq \hat{\beta_i}; \textrm{ else }E_{Api,logit-diff} = 0
...
\begin{equation}
\ R=min(\hat{\theta_p}), \textrm{ if and only if } min(\hat{\theta_p}) < min(\hat{\beta_i}); \textrm{ else }min(\hat{\beta_i})
(\#eq:R-eq)
\end{equation}
...
I explored the relationships visually between the actual effort calculated from known strengths and loads (i.e., equation \@ref(eq:EApi-SL-eq)) and each of the approaches to deriving effort from the Rasch model parameter estimates. For the approaches to deriving effort that yielded ratios of demands to capacity (i.e., equations \@ref(eq:EApi-mean-sigma-eq), \@ref(eq:EApi-odds-eq), and \@ref(eq:EApi-logit-shift-eq)) such that they were on the scale $[0,100]$ comparably with equation \@ref(eq:EApi-SL-eq), I also calculated the MADP for the efforts calculated for each strength and load pairing $Q$ between:
...
Where $\hat{E}_{Api}$ is the derived effort (i.e., equations \@ref(eq:EApi-mean-sigma-eq), \@ref(eq:EApi-odds-eq), or \@ref(eq:EApi-logit-shift-eq)) dependent on which scaling procedure is used.
...
Two of the N-back datasets included self-report ratings of the perception of effort required to perform the N-back task items using the NASA-TLX effort subscale [@westbrook_what_2013; @beh_maus_2021]. I fit an ordered beta regression model, appropriate to bounded data such as effort [@kubinec_ordered_2022; @steele_perceptions_2022], of the perception of effort with each actual effort operationalisation using random intercepts and slopes for each person. Examining the relationship between the estimated actual effort and the perception of effort here  would likely lead to different conclusions about the nature of the psychophysical relationship (figure \@ref(fig:nback-effort-NASA-ordbeta-plot)). Indeed, given the simulations and the results of analysing this empirical dataset, it would certainly seem that specific empirical conditions might be required to employ these approaches to deriving effort for cognitive tasks. 
...
[^12]: For example, as Michell [-@michell_introduction_1990] explains, finding $x_2$ that satisfies solvability may be impractical due to resource constraints. Also, the steps to show a certain value is bounded and restricted by the Archimedian condition may be practically impossible; for example, demonstrating the sun is a finite distance from the earth using a common 1 foot ruler [@domingue_evaluating_2014]. 

[^13]: Though Scott's [-@scott_measurement_1964] finite set of cancellation conditions can be used to directly test these axioms, though they are still empirically determined regarding the extent to which they test them.
